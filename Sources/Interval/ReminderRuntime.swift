...
enum UserIdleMonitor {
    static var idleSeconds: TimeInterval {
        let types: [CGEventType] = [.keyDown, .keyUp, .mouseMoved, .leftMouseDown, .leftMouseUp,
...
@MainActor final class ReminderOverlayController {
    ...
    func update(_ overlay: ReminderOverlay?, reminder: Reminder?, store: AppStore) {
        ...
        case .warning:
            ...
            configure(panel); panel.level = .floating; panel.contentView = NSHostingView(rootView: ReminderWarningView(reminder: reminder, overlay: overlay))
            panel.ignoresMouseEvents = true
            panels = [panel]; positionWarning(); panel.orderFrontRegardless()
        case .reminder:
            let cursor = NSEvent.mouseLocation
            let cursorScreen = NSScreen.screens.first(where: { $0.frame.contains(cursor) }) ?? NSScreen.main
            let screens = reminder.presentation == .fullscreen ? NSScreen.screens : [cursorScreen].compactMap { $0 }
            panels = screens.map { screen in
                let rect = reminder.presentation == .fullscreen ? safeFrame(for: screen) : centered(size: NSSize(width: 520, height: 480), in: screen.visibleFrame)
                let panel = EscapePanel(contentRect: rect, styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
                configure(panel); panel.level = .floating
                panel.contentView = NSHostingView(rootView: ReminderTakeoverView(reminder: reminder,
                    dismiss: { store.dismissReminder(reminder.id) }, snooze: { store.snoozeReminder(reminder.id) }))
                panel.onEscape = { store.dismissReminder(reminder.id) }
                panel.collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary]
                panel.sharingType = .none
                if screen == cursorScreen { panel.makeKeyAndOrderFront(nil) } else { panel.orderFrontRegardless() }
                return panel
            }
        }
    }
    ...
    private func centered(size: NSSize, in rect: NSRect) -> NSRect { NSRect(x: rect.midX-size.width/2, y: rect.midY-size.height/2, width: size.width, height: size.height) }
