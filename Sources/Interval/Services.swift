...
@MainActor final class NotificationService: NSObject, UNUserNotificationCenterDelegate {
    ...
    func schedule(timer: TimerState) {
        ...
        content.body = timer.kind == .focus ? "Your work is saved. Take a break when ready." : "Ready to focus?"
        content.sound = .default
        center.add(UNNotificationRequest(identifier: timer.id.uuidString, content: content,
            trigger: UNTimeIntervalNotificationTrigger(timeInterval: max(1, deadline.timeIntervalSinceNow), repeats: false))) { [weak self] error in
    ...
    nonisolated func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification,
        withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) { completionHandler([.banner, .sound]) }
}
...
@MainActor final class AmbientAudio {
    ...
    func play(_ sound: AmbientSound, volume: Double) throws {
        ...
        let engine = AVAudioEngine(), format = AVAudioFormat(standardFormatWithSampleRate: 44_100, channels: 1)!
        var seed: UInt64 = 0x12345678, sample: Float = 0, phase: Float = 0
        let source = AVAudioSourceNode { _, _, frames, list -> OSStatus in
            ...
            for frame in 0..<Int(frames) {
                ...
                let value: Float
                switch sound {
                case .brownNoise: sample = sample * 0.98 + white * 0.02; value = sample * 3
                case .rain: value = white * (abs(white) > 0.72 ? 0.8 : 0.12)
                case .ocean: phase += 0.00012; sample = sample * 0.96 + white * 0.04; value = sample * (0.25 + 0.18 * sin(phase))
                case .silence: value = 0
                }
                for buffer in buffers { buffer.mData?.assumingMemoryBound(to: Float.self)[frame] = value * Float(volume) }
            }
            return noErr
        }
        ...
        configurationObserver = NotificationCenter.default.addObserver(
            ...
            Task { @MainActor in
                ...
                self.stop()
                self.failure?("Audio output changed. Ambient sound was stopped to avoid switching speakers unexpectedly.")
            }
        }
    }
}
