...
final class CalendarService {
  ...
  init(fixtureEvents: [CalendarEventSnapshot]? = nil) {
    ...
    if let fixtureEvents {
      ...
      calendars = Array(Set(fixtureEvents.map(\.calendarName))).sorted().map {
        CalendarChoice(id: $0, title: $0)
      }
  ...
  func hasEvent(at date: Date) -> Bool {
    ...
    return todayEvents.contains {
      $0.isEligibleForReminderSuppression && $0.start <= date && $0.end > date
    }
  }
  ...
  func events(on day: Date, calendar: Calendar = .autoupdatingCurrent) -> [CalendarEventSnapshot] {
    guard isEnabled, authorizationState == .fullAccess,
      let interval = CalendarDates.dayInterval(containing: day, calendar: calendar)
    else { return [] }
  ...
  private func refreshHistory() {
    guard
      let interval = CalendarDates.monthInterval(
        containing: displayedMonth, calendar: .autoupdatingCurrent)
    else { return }
    query(interval: interval) { [weak self] events in self?.historyEvents = events }
  }

  private func refreshToday(containing date: Date) {
    guard let interval = CalendarDates.dayInterval(containing: date, calendar: .autoupdatingCurrent)
    else {
      todayEvents = []
      todayInterval = nil
      return
  ...
  private func query(
    interval: DateInterval, completion: @escaping @MainActor ([CalendarEventSnapshot]) -> Void
  ) {
  ...
  private static func snapshot(_ event: EKEvent) -> CalendarEventSnapshot {
    ...
    if event.status == .canceled {
      status = .canceled
    } else if event.attendees?.first(where: \.isCurrentUser)?.participantStatus == .declined {
      status = .declined
    ...
    return CalendarEventSnapshot(
      id: event.eventIdentifier ?? UUID().uuidString, title: event.title ?? "Untitled event",
