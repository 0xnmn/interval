    ...
    HSplitView {
      VStack(spacing: 0) {
        ...
        HStack {
          Menu {
            ...
            Label("Add from Template", systemImage: "plus")
          }
          Spacer()
      ...
      if let selection, let reminder = store.data.reminders.first(where: { $0.id == selection }) {
        ...
        ContentUnavailableView(
          "No reminders", systemImage: "bell",
          description: Text("Add a blank reminder or choose one of four customizable templates."))
      } else {
        ContentUnavailableView("Select a reminder", systemImage: "bell")
      }
...
struct ReminderWarningView: View {
  ...
  var body: some View {
    ...
    }.padding(18).frame(maxWidth: .infinity, maxHeight: .infinity).background(
...
struct ReminderTakeoverView: View {
  ...
  var body: some View {
    VStack(spacing: 18) {
      ...
      HStack {
        Button("Postpone this time · 5min", action: snooze)
        Button("Dismiss", action: dismiss).buttonStyle(.borderedProminent).keyboardShortcut(
          .cancelAction)
      }
      Text("Closes after \(Int(reminder.displaySeconds)) seconds. Escape dismisses it.").font(
        .caption
