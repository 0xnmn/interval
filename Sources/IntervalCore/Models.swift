...
public enum TimerStatus: String, Codable, Sendable {
  case ready, running, paused, completed, abandoned
}
...
public struct IntervalSettings: Codable, Equatable, Sendable {
  ...
  public func duration(for kind: TimerKind) -> TimeInterval {
    TimeInterval(
      (kind == .focus ? focusMinutes : kind == .shortBreak ? shortBreakMinutes : longBreakMinutes)
        * 60)
  }
  ...
  public init(from decoder: Decoder) throws {
    ...
    calendarIntegrationEnabled =
      try c.decodeIfPresent(Bool.self, forKey: .calendarIntegrationEnabled) ?? false
    selectedCalendarIDs =
      try c.decodeIfPresent(Set<String>.self, forKey: .selectedCalendarIDs) ?? []
    didChooseInitialCalendars =
      try c.decodeIfPresent(Bool.self, forKey: .didChooseInitialCalendars) ?? false
  }
}
...
public struct CalendarEventSnapshot: Identifiable, Equatable, Sendable {
  ...
  public init(
    id: String, title: String, start: Date, end: Date, allDay: Bool, calendarName: String,
...
public enum CalendarDates {
  public static func monthGrid(containing date: Date, calendar: Calendar) -> [Date?] {
    ...
    result += days.compactMap { calendar.date(byAdding: .day, value: $0 - 1, to: month.start) }.map(
      Optional.some)
...
public struct Reminder: Identifiable, Codable, Equatable, Sendable {
  ...
  public init(
    id: UUID = UUID(), title: String, message: String = "Time for a short break.",
    emoji: String = "⏱️",
  ...
  public static func templates(startingAt date: Date = Date()) -> [Reminder] {
    [
      ...
      Reminder(
        title: "Stretch", message: "Stand up and stretch gently.", emoji: "🙆",
        intervalSeconds: 1_800, displaySeconds: 60, dueAt: date.addingTimeInterval(1_800)),
      Reminder(
