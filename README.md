...
- **Focus cycle:** 25-minute focus sessions alternate with 5-minute breaks, with a 10-minute long break after every four completed focus sessions. All four values are configurable. Focus completion waits for feedback; Continue starts the break. Breaks remain in overtime until Resume focus starts the next session or Abandon ends the cycle. There is no pause. Taking a break ends unfinished focus with confirmation.
- **Native notifications:** macOS delivers session-end alerts and an optional one-minute focus heads-up (Settings → General). When a break ends, a native reminder with a Resume focus action repeats every five minutes until focus resumes or the break is abandoned. Interval must be running to register the follow-up when the break ends; if quit beforehand, registration waits until reopening. macOS notification permissions and Focus settings govern delivery. The in-app, menu-bar, and notch reflection forms remain available; custom automatic session popup windows have been removed.
- **Notch heads-ups:** with the notch enabled, it expands one minute before focus ends or an eligible reminder is due. Choose +5, +10, or +15 minutes to extend focus or delay just that reminder occurrence. It closes after ten seconds, stays usable while hovered or editing, and does not reopen for the same occurrence unless rescheduled outside the one-minute window. Suppressed and idle-paused reminders do not prompt. Ended breaks still keep the notch expanded. Presentation policy lives in `NotchHeadsUpState`; `NotchController` owns the native panel/animation, `AppStore` validates and applies actions, and `TimeAdjustmentChoices` is shared with timer menus. All use the existing application clock and reminder eligibility rules.
- **Time controls:** the dial is a fixed 60-minute clock: 25 minutes fills 150°, with a hand at the 25-minute mark. The numeric countdown and start/end times appear below it. Click ± for five minutes; right-click for 5-, 10-, and 15-minute choices. New intervals and adjustments are capped at one hour of total planned time. Legacy running intervals keep their existing deadlines but cannot be extended further.
...
- **To-dos:** one persistent checklist across every session. The home sidebar has a compact Today summary and separate, resizable To-dos and Upcoming reminders sections with pinned matching headers and independent scrolling. Long checklists never push reminders offscreen. Return inserts the next row, ↑/↓ navigate, and Backspace removes an empty row while keeping at least one editable item. Check off items or delete them from their context menu; the menu bar also adds to-dos. Existing scratchpad lines migrate to unchecked items. Session reflections remain separate.
...
- **Timeline and Stats:** the calendar timeline lives in Stats, not on the home screen. The reusable `DayTimeline` component combines saved sessions and Apple Calendar events on an hourly grid, with overlapping lanes, all-day events, and session inspection. Stats shows one selected day: navigate the week strip or use the date picker for any date. Category bars show focus-time distribution; feedback counts distinguish Focused, Neutral, Distracted, and Unrated. Category filters apply to that day's focus totals and logs, never to calendar events. Breaks are excluded from focus totals and feedback. A persistent live timer bar remains on Stats and Reminders, independent of the date being viewed, with start/break/abandon and reflection actions.
...
- Active microphone input suppresses reminder warnings, fullscreen reminders, and reminder notch heads-ups, including reminders already showing. Missed occurrences are advanced without queuing a burst after the call; only fresh post-call idle time counts toward warnings. Core Audio input-process activity and AVFoundation audio-device usage metadata provide independent signals without recording or opening the microphone. This also covers recording/dictation apps; output-only playback does not count. Muted calls without an active input stream depend on calendar suppression: the event must be available through Apple Calendar, in a selected calendar, with full-access permission. All-day events remain excluded.
