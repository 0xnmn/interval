...
- **To-dos:** one persistent checklist across every session, alongside today's stats and calendar. Add, edit, check off, or delete items; the menu bar also adds to-dos. Existing scratchpad lines migrate to unchecked items. Session reflections remain separate.
...
- **Timeline and Stats:** the dashboard combines today's saved sessions and Apple Calendar events chronologically. Stats filters sessions by category and shows daily category focus totals, active duration, and outcome. Calendar events remain separate from focus records and are never included in focus totals.
