...
- Active microphone input counts as activity, just like typing or moving the mouse: the warning waits, and only idle time after input stops counts toward its countdown. Core Audio reports input-stream use without recording audio or opening the microphone. This also covers recording/dictation apps; output-only playback does not count. A call that does not use an input stream cannot be detected this way. Already-visible reminders and explicit previews behave as they do for keyboard/mouse activity.
