...
struct CalendarEventTests {
    ...
    @Test func overlapUsesHalfOpenBounds() throws {
        ...
        #expect(event(start: interval.start.addingTimeInterval(-60), end: interval.start).overlaps(interval) == false)
        #expect(event(start: interval.end, end: interval.end.addingTimeInterval(60)).overlaps(interval) == false)
        #expect(event(start: interval.start, end: interval.start.addingTimeInterval(1)).overlaps(interval))
    }
    ...
    @Test func allDayMultiDayOverlapsEachCoveredDayOnly() throws {
        ...
        #expect(event.overlaps(try #require(CalendarDates.dayInterval(containing: first, calendar: calendar))))
        #expect(event.overlaps(try #require(CalendarDates.dayInterval(containing: calendar.date(byAdding: .day, value: 1, to: first)!, calendar: calendar))))
        #expect(!event.overlaps(try #require(CalendarDates.dayInterval(containing: third, calendar: calendar))))
    }
    ...
    @Test func canceledAndDeclinedDoNotSuppress() {
        ...
        #expect(!event(start: now, end: now.addingTimeInterval(60), status: .canceled).isEligibleForReminderSuppression)
        #expect(!event(start: now, end: now.addingTimeInterval(60), status: .declined).isEligibleForReminderSuppression)
        #expect(event(start: now, end: now.addingTimeInterval(60), status: .tentative).isEligibleForReminderSuppression)
    }
    ...
    private func event(start: Date, end: Date, allDay: Bool = false,
                       status: CalendarEventStatus = .confirmed) -> CalendarEventSnapshot {
        CalendarEventSnapshot(id: "event", title: "Event", start: start, end: end,
