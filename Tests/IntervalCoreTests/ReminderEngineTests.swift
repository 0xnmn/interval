...
@Suite("Reminder engine") struct ReminderEngineTests {
    ...
    func reminder(due: TimeInterval = 10) -> Reminder {
        Reminder(title: "Water", intervalSeconds: 60, displaySeconds: 5, dueAt: zero.addingTimeInterval(due))
    }
    ...
    @Test func activityPausesCountdownAndActualElapsedResumesIt() {
        var values = [reminder()]; var engine = ReminderEngine()
        _ = engine.tick(reminders: &values, now: zero, environment: .init(isUserIdle: true))
        _ = engine.tick(reminders: &values, now: zero.addingTimeInterval(3), environment: .init(isUserIdle: true, idleSeconds: 3))
        #expect(engine.overlay == .warning(reminderID: values[0].id, remaining: 7, isPaused: false))
        _ = engine.tick(reminders: &values, now: zero.addingTimeInterval(8), environment: .init(isUserIdle: false))
        #expect(engine.overlay == .warning(reminderID: values[0].id, remaining: 7, isPaused: true))
        _ = engine.tick(reminders: &values, now: zero.addingTimeInterval(15), environment: .init(isUserIdle: true, idleSeconds: 7))
    ...
    @Test func suppressionAndInactiveSessionSkipAndCoalesce() {
        var values = [reminder(due: -125)]; var engine = ReminderEngine()
        _ = engine.tick(reminders: &values, now: zero, environment: .init(focusIsRunningOrPaused: true))
        #expect(values[0].dueAt == zero.addingTimeInterval(55)); #expect(engine.overlay == nil)
        values[0].dueAt = zero.addingTimeInterval(-1)
        _ = engine.tick(reminders: &values, now: zero, environment: .init(isSessionActive: false))
        #expect(values[0].dueAt == zero.addingTimeInterval(59)); #expect(engine.overlay == nil)
    }

    @Test func deterministicEarliestAndNoOverlap() {
        var late = reminder(due: 9); late.id = UUID(uuidString: "ffffffff-ffff-ffff-ffff-ffffffffffff")!
        var early = reminder(due: 8); early.id = UUID(uuidString: "00000000-0000-0000-0000-000000000000")!
        var values = [late, early]; var engine = ReminderEngine()
    ...
    @Test func staleBacklogAdvancesInConstantTimeAndOverdueOccurrenceGetsFullWarning() {
        var stale = reminder(due: -1_000_000_000); var engine = ReminderEngine()
        var values = [stale]
        _ = engine.tick(reminders: &values, now: zero, environment: .init(isSessionActive: false))
        #expect(values[0].dueAt! > zero)

        stale = reminder(due: -1)
        values = [stale]; engine = ReminderEngine()
        _ = engine.tick(reminders: &values, now: zero, environment: .init(isUserIdle: true, idleSeconds: 100))
    ...
    @Test func longSamplingGapDoesNotReceiveUnverifiedIdleCredit() {
        var values = [reminder()]; var engine = ReminderEngine()
        _ = engine.tick(reminders: &values, now: zero, environment: .init(isUserIdle: true))
        _ = engine.tick(reminders: &values, now: zero.addingTimeInterval(5), environment: .init(isUserIdle: true))
    ...
    @Test func suppressedEarliestDoesNotBlockEligibleReminder() {
        var suppressed = reminder(due: 2); suppressed.suppressDuringFocus = true
        var eligible = reminder(due: 3); eligible.suppressDuringFocus = false
        var values = [suppressed, eligible]; var engine = ReminderEngine()
        _ = engine.tick(reminders: &values, now: zero, environment: .init(focusIsRunningOrPaused: true))
        #expect(engine.overlay?.reminderID == eligible.id)
    }

    @Test func legacyReminderDecodesDefaults() throws {
        let id = UUID(); let json = "{\"id\":\"\(id)\",\"title\":\"Old\",\"isEnabled\":true}"
        let value = try JSONDecoder().decode(Reminder.self, from: Data(json.utf8))
        #expect(value.message == "Time for a short break."); #expect(value.intervalSeconds == 1200)
        #expect(value.presentation == .floating); #expect(value.suppressDuringFocus)
    }
}
