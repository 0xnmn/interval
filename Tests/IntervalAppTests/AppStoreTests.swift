...
struct AppStoreTests {
    private func withStore(_ body: (AppStore, JSONStore) throws -> Void) throws {
        ...
        let store = AppStore(persistence: persistence, calendarService: CalendarService(fixtureEvents: []), runtimeEnabled: false)
    ...
    @Test func fourCompletionsOfferLongBreakExactlyOnce() throws {
        try withStore { store, persistence in
            for count in 1...4 {
                ...
                store.data.activeTimer = TimerState(kind: .focus, duration: 1500, status: .running,
    ...
    @Test func reflectionAndScratchpadPersistIndependently() throws {
        try withStore { store, persistence in
            let now = Date()
            let session = SessionRecord(timerID: UUID(), kind: .focus, startedAt: now.addingTimeInterval(-1500),
    ...
    @Test func corruptPersistenceNeverGetsOverwritten() throws {
        ...
        let store = AppStore(persistence: JSONStore(fileURL: file), calendarService: CalendarService(fixtureEvents: []), runtimeEnabled: false)
    ...
    @Test func exportContainsLocalDataButNoCalendarContents() throws {
        try withStore { store, persistence in
            ...
            let url = persistence.fileURL.deletingLastPathComponent().appendingPathComponent("export.json")
    ...
    @Test func selectedCalendarAndMidnightCacheAreIndependentOfHistory() {
        ...
        let work = CalendarEventSnapshot(id: "work", title: "Tomorrow", start: tomorrow,
            end: tomorrow.addingTimeInterval(600), allDay: false, calendarName: "Work")
        let personal = CalendarEventSnapshot(id: "personal", title: "Today", start: midnight,
